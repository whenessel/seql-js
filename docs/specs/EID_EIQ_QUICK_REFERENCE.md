# EID vs EIQ — Quick Reference

## 📋 Определения

```
┌─────────────────────────────────────────────────────┐
│  EID = Element Identity Descriptor                  │
│  Type: Structured JSON / AST                        │
│  Role: Source of Truth, Canonical Format            │
│  Usage: Internal library operations                 │
└─────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────┐
│  EIQ = Element Identity Query                       │
│  Type: Canonical string                             │
│  Role: Transport Format, Human Interface            │
│  Usage: Between systems (GA, storage, aggregation)  │
└─────────────────────────────────────────────────────┘
```

## 🔑 Ключевое правило

```typescript
// ✅ ПРАВИЛЬНО
EIQ → parse() → EID → resolve() → Element[]

// ❌ НЕПРАВИЛЬНО  
EIQ → resolve() → Element[] // NO AST!
```

**EIQ никогда не интерпретируется напрямую.**

## 📊 Flow диаграмма

```
rrweb Recording          Analytics (GA)         Backend              Player Replay
     │                        │                     │                      │
     │ Generate EID           │                     │                      │
     ├─────────────────▶ EID  │                     │                      │
     │                        │                     │                      │
     │ Serialize              │ Stringify           │                      │
     ├────────────────────────┼─────────────▶ EIQ  │                      │
     │                        │                     │                      │
     │                        │ Send to GA          │ Aggregate by EIQ     │
     │                        ├─────────────────────┼─────────────▶ EIQ   │
     │                        │                     │                      │
     │                        │                     │              Parse   │
     │                        │                     │              ├─▶ EID │
     │                        │                     │                      │
     │                        │                     │              Resolve │
     │                        │                     │              └─▶ Elements
```

## ✅ Требования к EIQ

| Требование | Описание | Почему важно |
|------------|----------|--------------|
| **Детерминированность** | Один EID → всегда один EIQ | Стабильная агрегация |
| **Каноничность** | Один EID → только один EIQ | Нет дубликатов в аналитике |
| **Версионированность** | `v1:footer > ul` | Обратная совместимость |
| **Stable-sort** | Алфавитный порядок атрибутов | Детерминированность |
| **PII-safe** | Нет email, имен и т.д. | GDPR compliance |

## ❌ Запреты

```typescript
// ❌ Использовать EIQ как CSS
document.querySelectorAll(eiq);

// ❌ Resolve без parse
resolve(eiq, dom);

// ❌ Хранить только EIQ
node.eiq = "..."; // Потеря данных!

// ❌ Несколько EIQ для одного EID
eiq1 = "footer>ul"; // ← Разные строки
eiq2 = "footer > ul"; // ← для одного EID
```

## 🎯 API

```typescript
// Stringify
function stringifyEID(eid: EID): EIQ;

// Parse
function parseEIQ(eiq: EIQ): EID;

// Resolve (только EID!)
function resolve(eid: EID, root: Document): Element[];
```

## 💡 Примеры

### rrweb Recording
```typescript
const eid = generateEID(element);
node.eid = eid; // Full
node.eiq = stringifyEID(eid); // Optional
```

### Analytics
```typescript
const eiq = stringifyEID(eid);
gtag('event', 'click', { element_identity: eiq });
```

### Backend
```typescript
const grouped = groupBy(events, e => e.element_identity);
// grouped[eiq] = { count: 245, sessions: 128 }
```

### Player
```typescript
const eid = parseEIQ(eiq);
const elements = resolve(eid, rrdom);
highlight(elements[0]);
```

## 📖 Формула

```
EID — это то, что система понимает
EIQ — это то, что системы обменивают

EID = Understanding
EIQ = Communication
```

---

См. `EID_VS_EIQ_ARCHITECTURE.md` для полной документации.
