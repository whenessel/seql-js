...
### Гипотеза 2: DSL генератор работает на неправильном входном элементе
...
### Шаг 1: Выполнить скрипт из ANALYZE_STRUCTURE.md
...
5. Found by text "18" in .rdp-day:
   ...
   Same as button in TD: false  ← ⚠️ ПРОБЛЕМА ЗДЕСЬ!

Found element position: Row X, Cell Y  ← неправильная позиция
```

### Шаг 2: Выполнить скрипт из DEBUG_WRONG_ELEMENT.md

Этот скрипт детально сравнит все три элемента.
...
### Решение 1: Фильтровать серые даты
...
### Решение 3: Улучшить поиск по позиции в таблице

```javascript
// Найти table, затем конкретную строку и ячейку
...
1. ✅ Открыть консоль браузера на странице с календарем
2. 📋 Выполнить скрипт из ANALYZE_STRUCTURE.md
3. 📋 Выполнить скрипт из DEBUG_WRONG_ELEMENT.md
4. 📊 Проанализировать вывод
5. 🔧 Определить правильное решение
6. 🛠️ Исправить код в dom-dsl пакете

## Файлы для использования

- **ANALYZE_STRUCTURE.md** - Детальный анализ структуры DOM
- **DEBUG_WRONG_ELEMENT.md** - Сравнение правильного и найденного элементов
- **MANUAL_TEST_COMMANDS.md** - Оригинальные тестовые команды
