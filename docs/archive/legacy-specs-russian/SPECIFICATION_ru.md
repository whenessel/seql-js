...
<div data-testid="checkout-form">
  <div data-qa="footer-contacts"></div>
</div>
```
...
const UTILITY_PATTERN =
  /^(p|m|px|py|pt|pb|pl|pr|mt|mb|ml|mr|gap|grid|flex|col|row|w|h|text|bg|border|rounded|shadow|opacity|z|top|bottom|left|right)[-:]/;
...
function calculateConfidence(dsl) {
  ...
  return anchorScore * 0.4 + avgPathScore * 0.3 + targetScore * 0.2 + uniquenessBonus * 0.1;
}
```
